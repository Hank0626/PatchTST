...
class SupervisedRunner(BaseRunner):
    ...
    def train_epoch(self, epoch_num=None):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            outputs = self.model(X.to(self.device), padding_masks)
            # ---------1. label loss----------------
            predictions = outputs['outputs_time']
            loss_label = self.loss_module(predictions, targets)  # (batch_size,) loss for each sample in the batch
            loss_label = loss_label.sum()/len(loss_label)
            # ---------2. feature loss---------------
            outputs_text, outputs_time, intermidiate_feat_time, intermidiate_feat_text \
                = outputs['outputs_text'], outputs['outputs_time'], outputs['intermidiate_time'], outputs[
                'intermidiate_text']
            loss_feature = sum([(0.8 ** idx) * F.l1_loss(feat_time, feat_text)
                                for idx, (feat_time, feat_text)
                                in enumerate(zip(intermidiate_feat_time[::-1], intermidiate_feat_text[::-1]))])
            # ---------3. logits loss ---------------------
            outputs_time_log_softmax = F.log_softmax(outputs_time,dim=-1)
    ...
    def evaluate(self, epoch_num=None, keep_all=True):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            predictions = predictions['outputs_time']
